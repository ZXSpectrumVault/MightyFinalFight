...
__inline void init_ie_help() {}

__inline void done_ie_help()
{
...
void showhelppp(char *anchor = 0) //Alone Coder 0.36.6
{
   ...
   if (!Show || !CreateMoniker) { MessageBox(fgwin, "Install IE4.0 or higher to view help", 0, MB_ICONERROR); return; }

   char dst[0x200]; GetTempPath(sizeof dst, dst); strcat(dst, "us_help.htm");
   ...
   for (;;) {
      ...
      if (x == '{') {
         ...
         while (r < sizeof(tag)-1 && !feof(ff) && (x = getc(ff)) != '}') tag[r++] = x;
         tag[r] = 0;
         if (tag[0] == '?') {
            RECT rc; GetWindowRect(fgwin, &rc);
            if (tag[1] == 'x') fprintf(gg, "%d", rc.right-rc.left-20);
            if (tag[1] == 'y') fprintf(gg, "%d", rc.bottom-rc.top-20);
         } else {
   ...
   char url[0x200]; sprintf(url, "file://%s%s%s", dst, anchor?"#":nil, anchor?anchor:nil);
   WCHAR urlw[0x200]; MultiByteToWideChar(AreFileApisANSI()? CP_ACP : CP_OEMCP, MB_USEGLYPHCHARS, url, -1, urlw, sizeof urlw / sizeof WCHAR);
   IMoniker *pmk = 0; CreateMoniker(0, urlw, &pmk);

   if (pmk) {
      temp.rflags = temp.rflags | RF_GDI; set_video();
      Show(fgwin, pmk, 0,0,0), pmk->Release();
      if (dbgbreak) temp.rflags = RF_MONITOR, set_video();
      else apply_video();
   }
